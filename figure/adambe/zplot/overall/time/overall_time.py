...
c = canvas(ctype, title='overall_time', dimensions=[800, 150])


######################################real_time#################################

t = table(file='overall_time.data')
d = drawable(canvas=c, xrange=[-0.6, t.getmax('rownumber')+0.6],
             yrange=[0.0015, 300000], dimensions=[pictureWidthLong, 100], coord=[30,30], yscale='log10')
...
series_list = ['AdaMBEFinder',	'FmbeFinder', 'PmbeFinder',	'ooMBEAFinder', 'ParAdaMBEFinder',	'ParMBE']
series_name = ['AdaMBE','FMBE','PMBE','ooMBEA','ParAdaMBE','ParMBE']
bgcolors    = ['white', 'white', 'white','white', 'lightcoral', 'thistle','sandybrown']
...
#### legend
L.draw(canvas=c, coord=[d.left()+1, d.top()+10], skipnext=1, skipspace=63,
