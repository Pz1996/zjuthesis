...
c = canvas(ctype, title='pruning_efficiency', dimensions=[400, 150])


######################################real_time#################################

t = table(file='pruning_efficiency.data')
d = drawable(canvas=c, xrange=[-0.6, t.getmax('rownumber')+0.6],
             yrange=[0, 200], dimensions=[pictureWidthShort, pictureHeight], coord=[40,30], yscale='linear')
