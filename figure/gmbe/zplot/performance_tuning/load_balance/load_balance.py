...
c = canvas(ctype, title='load_balance', dimensions=[400, 150])


######################################real_time#################################

t_1 = table(file='load_balance_bx_t.data')
t_2 = table(file='load_balance_bx_b.data')
t_3 = table(file = "load_balance_bx_w.data")
d = drawable(canvas=c, xrange=[0, 1900],
             yrange=[0, 120], dimensions=[pictureWidthShort, pictureHeight], coord=[40,30])

axis(drawable=d,style='xy',ticstyle='in',
     doxmajortics=True,doymajortics=True,doylabels=False,doxlabels=False,
...
# p.line(drawable=d, table=t_1, xfield='time', yfield='sm',
       # linecolor='blue', linewidth=1, legend=L, legendtext="GMBE-T")

# p.line(drawable=d, table=t_2, xfield='time', yfield='sm',
       # linecolor='black', linewidth=1, legend=L, legendtext="GMBE-B")

p.line(drawable=d, table=t_3, xfield='time', yfield='sm',
       linecolor=lines[0][0], linewidth=lines[0][1], linedash=lines[0][2],legend=L, legendtext="GMBE-WARP")
# max line
...
#### legend
L.draw(canvas=c, coord=[d.left()+80, d.top()+12], skipnext=1, skipspace=65,
