...
c = canvas(ctype, title='union', dimensions=[400, 150])


######################################real_time#################################

t = table(file='union.data')
d = drawable(canvas=c, xrange=[-0.6, t.getmax('rownumber')+0.6],
             yrange=[0.001, 1000], dimensions=[250, 100], coord=[40,30], yscale='log10')
...
axis(drawable=d, style='box', ticstyle='in',
         ...
         ytitle='Runing time(s)', doylabels=True,ytitleshift=[10,0],
         linewidth=0.8,
         #yaxisposition=1,
...
#          xlabelrotate=20,
     ...
     xaxisposition=0.001, #yauto=['','',0.5], 
        # xlabelshift=[0, -1],
         ...
         xtitlesize=8,
         ytitlesize=8,
         ymanual=[['0.001', 0.001], ['0.1', 0.1], ['10', 10], ['1000', 1000]],
...
series_list = ['adv', 'naive']
series_name = ['GMBE', 'GMBE-W/O_UNION']
