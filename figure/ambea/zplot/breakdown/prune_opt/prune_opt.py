...
c = canvas(ctype, title='prune_opt', dimensions=[400, 200])


######################################real_time#################################

t = table(file='prune_opt.data')
d = drawable(canvas=c, xrange=[-0.6, t.getmax('rownumber')+0.6],
             yrange=[0.2, 5000], dimensions=[pictureWidthShort, pictureHeight], coord=[40,30], yscale='log10')

axis(drawable=d,style='y',ticstyle='in',
     doxmajortics=False,doymajortics=True,
     ymanual=[['1', 1], ['10', 10],['100', 100], ['1k', 1000]],
     ticmajorsize = 3,         
...
axis(drawable=d, style='box', ticstyle='in',
         ...
         ytitle='Ratio d/a', doylabels=True, ytitleshift=[ytitleShiftX,ytitleShiftY],
         linewidth=0.8,
         #yaxisposition=1,
...
series_list = ['AmbeaFinder', 'AmbeaFinderInter', 'IntraFinder','IntraFinderPassive','MBEAFinder']
series_name = ['AMBEA','AMBEA-ASE','AMBEA-AMP','AMBEA-PMP','MBEA']
bgcolors    = ['white', 'white', 'white', 'white', 'white','lightgrey', 'lightgrey', 'black']
fillcolors  = ['lightcoral','wheat','darkseagreen','honeydew','whitesmoke']
...
#### legend
L.draw(canvas=c, coord=[d.left()+4, d.top()+18], skipnext=2, skipspace=103,
