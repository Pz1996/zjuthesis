...
c = canvas(ctype, title='optimization', dimensions=[600, 150])


######################################real_time#################################

t = table(file='optimization.data')
d = drawable(canvas=c, xrange=[-0.6, t.getmax('rownumber')+0.6],
             yrange=[0.002, 5000], dimensions=[550, 100], coord=[40,30], yscale='log10')
...
#          xlabelrotate=20,
     ...
     xaxisposition=0.001, #yauto=['','',0.5], 
        # xlabelshift=[0, -1],
         ...
         xtitlesize=8,
         ytitlesize=8,
         ymanual=[['0.01', 0.01], ['0.1', 0.1], ['1', 1], ['10', 10],['100', 100], ['1000', 1000]],
...
series_list = ['adv', 'no_prune', 'WPT', 'BPT', 'no_union']
series_name = ['GMBE', 'GMBE-W/O_PRUNE', 'GMBE-WARP', 'GMBE-BLOCK', 'GMBE-W/O_UNION']
bgcolors    = ['white', 'white', 'white', 'white', 'white','lightgrey', 'lightgrey', 'black']
fillcolors  = ['darkgreen', 'lavenderblush', 'lightyellow', 'oldlace', 'azure','black', 'black', 'black',]
...
#### legend
L.draw(canvas=c, coord=[d.left()+5, d.top()+10], skipnext=1, skipspace=115,
