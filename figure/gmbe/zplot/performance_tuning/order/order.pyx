...
c = canvas(ctype, title='order', dimensions=[400, 150])


######################################real_time#################################

t = table(file='order.data')
d = drawable(canvas=c, xrange=[-0.6, t.getmax('rownumber')+0.6],
             yrange=[0.002, 30000], dimensions=[pictureWidthShort, pictureHeight], coord=[40,30], yscale='log10')
...
axis(drawable=d, style='box', ticstyle='in',
         ...
         ytitle='Running time (s)', doylabels=True,ytitleshift=[ytitleShiftX,-7],
         linewidth=0.8,
         #yaxisposition=1,
...
series_list = ['order1', 'order0', 'order3']
series_name = ['GMBE-INC','GMBE-RAND','GMBE-UC']
bgcolors    = ['white', 'white', 'white', 'white', 'white','lightgrey', 'lightgrey', 'black']
fillcolors  = ['darkgreen', 'lightcyan', 'lightyellow', 'black', 'black','black', 'black', 'black',]
...
#### legend
L.draw(canvas=c, coord=[d.left()+2, d.top()+10], skipnext=1, skipspace=100,
