...
c = canvas(ctype, title='memory', dimensions=[400, 150])


######################################real_time#################################

t = table(file='memory.data')
d = drawable(canvas=c, xrange=[-0.6, t.getmax('rownumber')+0.6],
             yrange=[0.02, 20000], dimensions=[pictureWidthShort, pictureHeight], coord=[40,30], yscale='log10')

axis(drawable=d,style='y',ticstyle='in',
     doxmajortics=False,doymajortics=True,
     ymanual=[['0.1', 0.1], ['1', 1], ['10', 10],['40', 40],['100', 100],['1k', 1000],['10k', 10000]],
     ticmajorsize = 3,         
...
axis(drawable=d, style='box', ticstyle='in',
         ...
         ytitle='Memory demand (GB)', doylabels=True,ytitleshift=[ytitleShiftX,ytitleShiftY],
         linewidth=0.8,
         #yaxisposition=1,
         #xaxisposition=0, yauto=['','',3],Figure~\ref{fig:exp_overall}
#          xlabelrotate=20,
...
series_list = ['reuse', 'noreuse']
series_name = ['GMBE', 'GMBE-W/O_REUSE']
bgcolors    = ['white', 'white', 'white', 'white', 'white','lightgrey', 'lightgrey', 'black']
fillcolors  = fig_colors_light
...
p.function(drawable=d, function=lambda x: 40,xrange=[-0.6, t.getmax('rownumber')+0.6], legend="", legendtext="memory capacity (40GB)", linedash=0, linecolor='red', step=0.1)
...
#### legend
L.draw(canvas=c, coord=[d.left()+20, d.top()+10], skipnext=1, skipspace=120,
