...
c = canvas(ctype, title='models', dimensions=[400, 150])


######################################real_time#################################

t = table(file='models.data')
d = drawable(canvas=c, xrange=[-0.6, t.getmax('rownumber')+0.6],
             yrange=[0.001, 3000], dimensions=[pictureWidthShort, pictureHeight], coord=[40,30], yscale='log10')
...
#          xlabelrotate=20,
     ...
     #xmanual = t.getaxislabels(column='Serie'),
...
series_list = ['WPT', 'BPT', 'Q']
series_name = ['GMBE-W', 'GMBE-B', 'GMBE-T']
...
#### legend
L.draw(canvas=c, coord=[d.left()+40, d.top()+10], skipnext=1, skipspace=55,
